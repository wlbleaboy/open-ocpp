                                     ...
                                     std::string&                  error_message)
{
    ...
    {
    data.certificateType = ocpp::types::ocpp21::CertificateSigningUseEnumTypeHelper.fromString(json["certificateType"].GetString());
    }

    // hashRootCertificate
    if (json.HasMember("hashRootCertificate"))
    {
    ocpp::types::ocpp21::CertificateHashDataTypeConverter hashRootCertificate_converter;
    ret = ret && hashRootCertificate_converter.fromJson(json["hashRootCertificate"], data.hashRootCertificate, error_code, error_message);
    }
...
bool SignCertificateReqConverter::toJson(const SignCertificateReq& data, rapidjson::Document& json) 
{
    ...
    {
    fill(json, "certificateType", ocpp::types::ocpp21::CertificateSigningUseEnumTypeHelper.toString(data.certificateType));
    }

    // hashRootCertificate
    if (data.hashRootCertificate.isSet())
    {
    ocpp::types::ocpp21::CertificateHashDataTypeConverter hashRootCertificate_converter;
    hashRootCertificate_converter.setAllocator(allocator);
    rapidjson::Document hashRootCertificate_doc(rapidjson::kObjectType);
    ret = ret && hashRootCertificate_converter.toJson(data.hashRootCertificate, hashRootCertificate_doc);
    json.AddMember(rapidjson::StringRef("hashRootCertificate"), hashRootCertificate_doc.Move(), *allocator);
    }
